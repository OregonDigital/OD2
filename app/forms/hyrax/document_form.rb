module Hyrax
  class DocumentForm < Hyrax::GenericForm
    ...
    def primary_terms
      required_fields + OregonDigital::DocumentMetadata::PROPERTIES.map(&:to_sym) + (OregonDigital::GenericMetadata::PROPERTIES.map(&:to_sym) - [:dcmi_type]) + [:keyword, :title, :creator] 
    end
  end
end
