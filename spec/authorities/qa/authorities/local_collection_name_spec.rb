...
RSpec.describe Qa::Authorities::LocalCollectionName do
  ...
  it { expect(local_collection_name_instance.label.call(response.first)).to eq 'mylabel' }
  describe '#search' do
    before do
      allow(local_collection_name_instance).to receive(:json).with(anything).and_return(response.first)
    end
