...
RSpec.describe FetchGraphWorker, type: :worker do
  ...
  let(:controlled_val_2) { Hyrax::ControlledVocabularies::Location.new('http://opaquenamespace.org/ns/creator/ChabreWayne') }
  let(:work) { model }
  ...
  describe '#perform' do
    context 'when the request works' do
      before do
        ...
        allow(controlled_val_2).to receive(:fetch).with(:anything).and_return({})
