...
RSpec.describe GenericIndexer do
  let(:solr_doc) { {} }
  let(:attributes) { {title: active_triple, dcmi_type: "MyType"} }
  let(:active_triple) { instance_double("ActiveTriples::Relation") }
  ...
  context "when #generate_solr_document is called" do
    before do
      ...
      allow(active_triple).to receive(:is_a?).and_return(true)
      allow(active_triple).to receive(:to_a).and_return(["MyTitle"])
    end
